...
board.on('ready', function ()
{
    ...
    // Run through the config again to setup the OneWire stuff
    // Run through the config again and setup the OneWire pin modes.
    if(typeof config.brewnoduino.OneWire != 'undefined' || config.brewnoduino.OneWire > 0)
    {
        ...
        /**
        board.sendOneWireSearch(config.brewnoduino.OneWire.port, function (err, devices)
        {
            if(err)
            {
                console.error(error);
            ...
            var readTemperature = function() {
                // start transmission
                board.sendOneWireReset(config.brewnoduino.OneWire.port);

                // a 1-wire select is done by ConfigurableFirmata
                board.sendOneWireWrite(config.brewnoduino.OneWire.port, device, 0x44);

                // the delay gives the sensor time to do the calculation
                board.sendOneWireDelay(config.brewnoduino.OneWire.port, 1000);

                // start transmission
                board.sendOneWireReset(config.brewnoduino.OneWire.port);

                // tell the sensor we want the result and read it from the scratchpad
                board.sendOneWireWriteAndRead(config.brewnoduino.OneWire.port, device, 0xBE, 9, function(error, data) {
                    if(error) {
                        console.error(error);
                        return;
                    }
